...
CLASS abap_unit_testclass IMPLEMENTATION.
  ...
  METHOD serialize_decfloat.
    " DECFLOAT16/DECFLOAT34 zero value must serialize as 0, not null (bug fix ported from Z_UI2_JSON)
  ...
  METHOD abap_to_json_simple_transform.
    ...
    IF <xt_json> IS ASSIGNED.
      ...
      rv_json = cl_abap_codepage=>convert_from( source = lv_jsonx ).
  ...
  METHOD serialize_associative_array.
    ...
    DATA: lt_table1  TYPE STANDARD TABLE OF ty_s_data,
          ...
          lt_table7  TYPE HASHED TABLE OF ty_s_data WITH UNIQUE KEY key1,
    ...
    cl_aunit_assert=>assert_not_initial( act = lv_act msg = 'Serialization of hashed table with unique key and formatting fails' ).

    ls_table-key1 = ')\n"PARAM/1"'.
    ls_table-key2 = 2.
    ls_table-value1 = 'value'.
    INSERT ls_table INTO TABLE lt_table7.

    lv_exp = '{")\\n\"PARAM/1\"":{"key2":2,"value1":"value"}}'.
    lv_act = serialize( data = lt_table7 pretty_name = pretty_mode-camel_case assoc_arrays = abap_true ).
    cl_aunit_assert=>assert_equals( act = lv_act exp = lv_exp msg = 'Serialization of sorted table with unique key and escaping fails' ).
    ...
    cl_aunit_assert=>assert_not_initial( act = lv_act msg = 'Serialization of hashed table with array optimization and formatting fails' ).
...
**********************************************************************
  ...
  METHOD deserialize_associative_array.
    ...
    CONCATENATE '{ "key1": { "value1" : "test1", "value2" : "test1" },'
                ...
                '  ")\\n\"PARAM/1\"": { "value1" : "test3", "value2" : "test3" } }'
    INTO lv_data.
  ...
  METHOD serialize_object.
    ...
    cl_aunit_assert=>assert_not_initial( act = lv_act msg = 'Serialization of simple recursive object with formatting fails' ).
...
*    ).
  ...
  METHOD deser_field_invalid_value.
    ...
    TRY.
        ...
        cl_abap_unit_assert=>fail( ).
      CATCH cx_sy_move_cast_error INTO lx_move.
        " kernel reader throws at next_node() before delivering field name — reduced path
        cl_aunit_assert=>assert_not_initial( act = lx_move->source_typename ).
    ENDTRY.
  ...
  METHOD generate_disable_type_detect.
    " a JSON string shaped like YYYY-MM-DD is inferred as ABAP date by default; with the
    " flag set it must stay a STRING. Assert on the GENERATED FIELD TYPE (what the flag changes).

    DATA: lo_json  TYPE REF TO z_ui2_json2,
          lr_data  TYPE REF TO data,
          lo_struc TYPE REF TO cl_abap_structdescr,
          lo_field TYPE REF TO cl_abap_datadescr.
    FIELD-SYMBOLS <struc> TYPE any.

    DATA(lv_json) = `{"id":"0133-01-01","clock":"12:30:00"}`.

    " default behaviour: date/time-shaped strings auto-detect to ABAP date (D) / time (T)
    CREATE OBJECT lo_json.
    lo_json->generate_int( EXPORTING json = lv_json CHANGING data = lr_data ).
    ASSIGN lr_data->* TO <struc>.
    lo_struc ?= cl_abap_typedescr=>describe_by_data( <struc> ).
    lo_field ?= lo_struc->get_component_type( 'ID' ).
    cl_abap_unit_assert=>assert_equals( exp = cl_abap_typedescr=>typekind_date act = lo_field->type_kind
      msg = 'default: YYYY-MM-DD should auto-detect as ABAP date' ).
    lo_field ?= lo_struc->get_component_type( 'CLOCK' ).
    cl_abap_unit_assert=>assert_equals( exp = cl_abap_typedescr=>typekind_time act = lo_field->type_kind
      msg = 'default: HH:MM:SS should auto-detect as ABAP time' ).

    " flag ON: both stay STRING
    CREATE OBJECT lo_json EXPORTING disable_string_type_detect = abap_true.
    lo_json->generate_int( EXPORTING json = lv_json CHANGING data = lr_data ).
    ASSIGN lr_data->* TO <struc>.
    lo_struc ?= cl_abap_typedescr=>describe_by_data( <struc> ).
    lo_field ?= lo_struc->get_component_type( 'ID' ).
    cl_abap_unit_assert=>assert_equals( exp = cl_abap_typedescr=>typekind_string act = lo_field->type_kind
      msg = 'flag: date-shaped id must stay STRING' ).
    lo_field ?= lo_struc->get_component_type( 'CLOCK' ).
    cl_abap_unit_assert=>assert_equals( exp = cl_abap_typedescr=>typekind_string act = lo_field->type_kind
      msg = 'flag: time-shaped clock must stay STRING' ).
